...
function getArtistByIndex(array, index) {
  const artistAtIndex = "the artist at index " + array[index].id + " is " + array[index].name;
  console.log(artistAtIndex);
...
function addArtist(artists) {
  const me = {
    ...
    bio: "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Ad architecto aliquam placeat et deserunt fugit provident in eum dignissimos, quisquam ea perspiciatis quibusdam, laborum voluptatem ex qui expedita? Voluptates, vel",
  }
  artists.push(me);
...
function lotsOfArt(array) {
  const lotOfArt = [];
  for(let i = 0; i < array.length; i++){
    if(Number(array[i].paintings) > 100){
      lotOfArt.push(array[i].name);
    }
  }
  console.log(lotOfArt);
  return lotOfArt;
}
lotsOfArt(artists);
